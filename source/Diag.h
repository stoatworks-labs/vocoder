...
/**
    ...
    `FF_FAIL` because one of the five shaders would not compile. From the
    operator's side that looks like "the effect does nothing", with no message
    anywhere -- so it records *which* one, which is the difference between a
    five-minute fix and an afternoon. The GL vendor and version strings go in
    next to it, because a shader that compiles on one machine and not on
    another is a driver answer, not a source answer. It also records, once,
    what the host's clock and its spectrum looked like, because the envelope
    followers' time constants depend on the first and nothing in FFGL says
    what unit either is in.
*/
namespace vocoder::diag
{
